...
real satisfying background, assembles the file and writes the publishing metadata.
...
2. The agent runs the six pipeline steps in order — one file each, in `steps/`.
...
## The pipeline — six pre-built steps
...
| 06 | Metadata         | `steps/06-metadata.md`     | `final.mp4` + `script.md` + `brief.md`    | `metadata.md` (title, description, tags, hashtags)     |
...
- **03 — Voiceover.** Turns the script into one clean narration file. **Default path
  ...
  when a key is present. The final duration becomes the contract for every later step.
...
- **06 — Metadata.** Title, description, tags, hashtags — optimized for the
...
| `channels/` | The channel library: one profile per channel (`<slug>.md`) + `_template.md`. Step 01 reads from here; step 06 appends history here. |
