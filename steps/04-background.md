...
| Log says | Cause | Fix |
|----------|-------|-----|
| `Sign in to confirm you're not a bot` | YouTube requires a **PO token** (proof-of-origin) for this client / the runner's IP is flagged | The workflow walks a client ladder automatically: default → `android,web_safari` → `ios` → `web_embedded,tv_embedded` → `mweb,tv_embedded`, and starts a local **bgutil POT-provider** container (`brainicism/bgutil-ytdlp-pot-provider` on :4416) that mints PO tokens for the yt-dlp plugin. If all fail: provide cookies via a `YT_COOKIES` repository secret (Netscape format, base64) — the workflow picks them up automatically if present. |
